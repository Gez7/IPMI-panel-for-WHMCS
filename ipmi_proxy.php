...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'var tHsOk=st&&st.transportHandshakeOk;'
        ...
        . 'if(tFailed&&!tHealthy&&!tHsOk){if(live&&sessR){try{_kvmDbg("ilo_strong_confirmation_rejected_transport_unhealthy",{tv:tv,live:1,sessR:1});}catch(_rtu){}return {ok:false,why:"transport_unhealthy_console_not_confirmed",transport_verdict:tv};}return {ok:false,why:"all_handshakes_failed",transport_verdict:tv};}'
        . 'if(tHealthy&&live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=3){try{_kvmDbg("ilo_strong_confirmation_transport_healthy",{tv:tv,frames:tFrames?1:0});}catch(_th){}return {ok:true,why:"console_transport_healthy"};}return {ok:false,why:"stabilizing_transport_healthy"};}'
        . 'if(tHsOk&&live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=5){try{_kvmDbg("ilo_strong_confirmation_handshake_ok",{tv:tv});}catch(_hsc){}return {ok:true,why:"handshake_ok_console_visible"};}return {ok:false,why:"stabilizing_handshake_ok"};}'
        . 'if(live&&sessR&&!mgmt&&!ov&&tv==="not_attempted"){st.finStable=(st.finStable||0)+1;if(st.finStable>=8){try{_kvmDbg("ilo_strong_confirmation_live_display_no_ws_attempted",{tv:tv,live:1,sessR:1});}catch(_nwa){}return {ok:true,why:"user_visible_console_success_no_ws"};}return {ok:false,why:"live_display_awaiting_transport"};}'
        . 'if(live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=10){try{_kvmDbg("ilo_strong_confirmation_live_display_transport_unclear",{tv:tv});}catch(_tuc){}return {ok:true,why:"user_visible_console_success_transport_unclear"};}return {ok:false,why:"live_display_transport_unclear"};}'
        ...
        . 'w.WebSocket=function(u,p){var isRelay=(typeof u==="string"&&(u.indexOf("ipmi_ws_relay.php")>=0||u.indexOf("wss://")>=0||u.indexOf("ws://")>=0));'
