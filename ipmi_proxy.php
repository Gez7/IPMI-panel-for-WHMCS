...
/**
 * Shared browser buglog client (token resolution works inside nested iframes where P lacks the session token).
 */
function ipmiProxyKvmBuglogClientCoreJs(): string
{
    return 'function kvmBugToken(){try{var m=String(P||"").match(/\\/([a-f0-9]{64})(?:\\/|$)/i);return m?m[1].toLowerCase():"";}catch(e){return"";}}'
        . 'function kvmBugTokenResolved(){try{var t=kvmBugToken();if(t&&t.length===64)return t;}catch(e){}try{var g=window;for(var d=0;g&&d<12;d++){var x=g.__ipmi_kvm_bug_token;if(typeof x==="string"&&/^[a-f0-9]{64}$/i.test(x))return String(x).toLowerCase();try{g=g.parent;}catch(z){break;}}}catch(e2){}try{var tp=window.top;if(tp&&tp!==window){var x2=tp.__ipmi_kvm_bug_token;if(typeof x2==="string"&&x2.length===64)return x2.toLowerCase();var p=String(tp.location&&tp.location.pathname||"");var m2=p.match(/\\/([a-f0-9]{64})(?:\\/|$)/i);if(m2)return m2[1].toLowerCase();}}catch(e3){}return"";}'
        . 'function kvmBugSyncTokenGlobally(){try{var t=kvmBugToken();if(t&&t.length===64){try{window.__ipmi_kvm_bug_token=t;}catch(e){}try{if(window.top)window.top.__ipmi_kvm_bug_token=t;}catch(e2){}}}catch(e3){}}'
        . 'function kvmBugSend(section,event,detail){try{if((!KVM_BUGLOG||!KVM_BUGLOG.run_id)&&window.top&&window.top!==window){try{var kb=window.top.__ipmi_kvm_buglog_plan;if(kb&&kb.run_id&&kb.ingest)KVM_BUGLOG=kb;}catch(e0){}}if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugTokenResolved();if(!tok||tok.length!==64)return;var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:String(section||"BROWSER"),event:String(event||""),detail:(detail!=null&&typeof detail==="object")?detail:{note:String(detail||"")}});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bb=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bb))return;}catch(e1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(e2){}}catch(e){}}'
        . 'function kvmBugFinalizeRun(extra){try{if((!KVM_BUGLOG||!KVM_BUGLOG.run_id)&&window.top&&window.top!==window){try{var kb2=window.top.__ipmi_kvm_buglog_plan;if(kb2&&kb2.run_id&&kb2.ingest)KVM_BUGLOG=kb2;}catch(eZ){}}if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugTokenResolved();if(!tok||tok.length!==64)return;var d=(extra!=null&&typeof extra==="object")?extra:{};var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:"FINAL",event:"kvm_run_finalize",detail:d});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bbF=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bbF))return;}catch(eF1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(eF2){}}catch(eF){}}';
}
...
{
    return 'try{(function(){'
        . 'if(window.__ipmi_kvm_cap)return;'
        . 'window.__ipmi_kvm_cap=1;'
        . 'function _seq(){try{return(window.__ipmi_kvm_console_seq=(window.__ipmi_kvm_console_seq|0)+1);}catch(e){return 1;}}'
        . 'function _emit(lv,src,ev,dt){try{if(typeof kvmBugSend!=="function")return;'
        . 'var d=(dt!=null&&typeof dt==="object")?dt:{msg:String(dt!=null?dt:"").substring(0,800)};'
        . 'kvmBugSend("BROWSER_CONSOLE","browser_console_line",{seq:_seq(),level:String(lv||"log"),source:String(src||"").substring(0,160),event:String(ev||"line"),detail:d});'
        . '}catch(e){}}'
        . 'try{window.__ipmi_kvm_console_emit=_emit;}catch(e){}'
        . 'function _wrap(n){try{var c=window.console;if(!c||!c[n])return;if(c[n].__ipmi_orig)return;var o=c[n];c[n]=function(){try{var a=[],i;for(i=0;i<arguments.length;i++){var x=arguments[i];try{a.push(typeof x==="string"?x:JSON.stringify(x));}catch(e){a.push(String(x));}}var s=a.join(" ");if(String(s).replace(/\\s+/g,"").length===0)return o.apply(c,arguments);_emit(n,"console."+n,"console_"+n,{msg:s.substring(0,900)});}catch(e){}return o.apply(c,arguments);};c[n].__ipmi_orig=o;}catch(e){}}'
        . '["log","warn","error","info","debug"].forEach(_wrap);'
        ...
        . 'try{if(typeof window.fetch==="function"&&!window.fetch.__ipmi_k){var _f0=window.fetch;window.fetch=function(inp,init){return _f0.apply(this,arguments).then(function(resp){try{if(resp&&(!resp.ok)&&resp.status>=400){var u="";try{u=(typeof inp==="string")?inp:(inp&&inp.url)||"";}catch(e){}u=String(u).substring(0,220);var low=String(u).toLowerCase();_emit("error","fetch","browser_fetch_"+String(resp.status),{url:u,status:resp.status});if(resp.status===502)_emit("error","fetch","browser_fetch_502",{url:u});if(resp.status===403){_emit("error","fetch","browser_fetch_403",{url:u,status:403});var sh=(low.indexOf("/sse/ui")>=0||low.indexOf("sse/ui")>=0);var si=(low.indexOf("session_info")>=0||low.indexOf("/json/session_info")>=0);if(sh||si){try{window.__ipmi_kvm_shell_bootstrap_dead=1;}catch(ebd){}try{if(typeof kvmBugSend==="function"){var _knd=(sh&&si)?"both":(sh?"sse_ui":"session_info");kvmBugSend("BROWSER","browser_shell_watchdog_forbidden",{url:u,status:403,kind:_knd});}}catch(ek){}}}}catch(e){}return resp;},function(err){try{_emit("error","fetch","browser_fetch_rejected",{msg:String(err&&err.message||err)});}catch(e){}throw err;});};window.fetch.__ipmi_k=1;}}catch(e){}'
        . 'try{if(window.XMLHttpRequest&&XMLHttpRequest.prototype&&!XMLHttpRequest.prototype.__ipmi_kxhr){var _xo=XMLHttpRequest.prototype.open;var _xs=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.open=function(m,u){try{this.__ipmi_xhr_u=u;}catch(e){}return _xo.apply(this,arguments);};XMLHttpRequest.prototype.send=function(){try{var self=this;if(!self.__ipmi_kxhr_le){self.__ipmi_kxhr_le=1;self.addEventListener("loadend",function(){try{var st=self.status|0;if(st>=400){var u="";try{u=String(self.__ipmi_xhr_u||self.responseURL||"");}catch(e2){}u=u.substring(0,220);var ux=String(u).toLowerCase();_emit("error","xhr","browser_fetch_"+String(st),{url:u,status:st});if(st===502)_emit("error","xhr","browser_fetch_502",{url:u});if(st===403){_emit("error","xhr","browser_fetch_403",{url:u,status:403});var xsh=(ux.indexOf("/sse/ui")>=0||ux.indexOf("sse/ui")>=0);var xsi=(ux.indexOf("session_info")>=0||ux.indexOf("/json/session_info")>=0);if(xsh||xsi){try{window.__ipmi_kvm_shell_bootstrap_dead=1;}catch(xbd){}try{if(typeof kvmBugSend==="function"){var _xk=(xsh&&xsi)?"both":(xsh?"sse_ui":"session_info");kvmBugSend("BROWSER","browser_shell_watchdog_forbidden",{url:u,status:403,kind:_xk,xhr:1});}}catch(xk){}}}}}catch(e3){}});}}catch(e4){}return _xs.apply(this,arguments);};XMLHttpRequest.prototype.__ipmi_kxhr=1;}}catch(e5){}'
...
function ipmiProxyBuildKvmAutoLaunchPreambleJs(string $familyJs, string $planJs, string $pxJs, string $autoJs, string $dbgLit): string
{
    return '(function(){'
        ...
        . 'var KSA="";try{if(PLAN&&PLAN.kvm_shell_abandon&&PLAN.kvm_shell_abandon.reason)KSA=String(PLAN.kvm_shell_abandon.reason||"");}catch(_ksa0){KSA="";}'
        . 'var KVM_BUGLOG=null;try{KVM_BUGLOG=(PLAN&&PLAN.kvm_buglog&&PLAN.kvm_buglog.run_id)?PLAN.kvm_buglog:null;}catch(_kb0){KVM_BUGLOG=null;}'
        . 'try{if(KVM_BUGLOG&&KVM_BUGLOG.run_id){window.__ipmi_kvm_buglog_plan=KVM_BUGLOG;try{if(window.top)window.top.__ipmi_kvm_buglog_plan=KVM_BUGLOG;}catch(_kbp){}}}catch(_kbq){}'
        ...
        . 'try{kvmBugSyncTokenGlobally();}catch(_bst0){}'
        . ipmiProxyKvmConsoleCaptureAndDomGuardJs()
        . 'try{if(typeof window.addEventListener==="function"){window.addEventListener("pagehide",function(){try{kvmBugSyncTokenGlobally();kvmBugFinalizeRun({reason:"pagehide",force_finalize:1});}catch(_ph){}},true);}}catch(_peh){}'
...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'function kvmWsRelaySeen(){try{var e=performance.getEntriesByType("resource"),i;for(i=0;i<e.length;i++){var n=String(e[i].name||"");if(n.indexOf("ipmi_ws_relay.php")>=0||n.indexOf("/wss/")>=0||n.indexOf("ircport")>=0)return true;}}catch(e0){}'
        . 'try{if(window.__ipmi_kvm_ws_relay_ts&&(kvmNow()-window.__ipmi_kvm_ws_relay_ts)<180000)return true;}catch(e1){}'
        . 'try{var cc=typeof collectContexts==="function"?collectContexts():[];for(var ci=0;ci<cc.length;ci++){try{if(cc[ci]&&cc[ci].__ipmi_kvm_ws_relay_ts&&(kvmNow()-cc[ci].__ipmi_kvm_ws_relay_ts)<180000)return true;}catch(_wc){}}}catch(e2){}'
        . 'return false;}'
        ...
        . 'function kvmIloWsHandshakeOk(){try{return !!(window.__ipmi_kvm_ws_handshake_ok&&window.__ipmi_kvm_ws_handshake_ok>0);}catch(e){return false;}}'
        . 'function kvmIloWsHandshakeFailed(){try{return !!(window.__ipmi_kvm_ws_handshake_failed&&window.__ipmi_kvm_ws_handshake_failed>0);}catch(e){return false;}}'
        . 'function kvmIloWsFrameFlowActive(){try{return !!(window.__ipmi_kvm_ws_frame_count&&window.__ipmi_kvm_ws_frame_count>0);}catch(e){return false;}}'
        . 'function kvmIloSustainedTransportEvidence(){try{var fc=window.__ipmi_kvm_ws_frame_count||0;var f0=window.__ipmi_kvm_ws_first_frame_ts||0;var last=window.__ipmi_kvm_ws_last_msg_ts||0;var now=kvmNow();if(fc<10)return false;if(!f0||!last)return false;if((now-last)>10000)return false;var span=last-f0;if(span<2500&&fc<16)return false;return true;}catch(e){return false;}}'
        . 'function kvmIloTransportHealthVerdict(){try{'
        . 'var attempted=window.__ipmi_kvm_ws_connect_attempted||0;'
        . 'var ok=window.__ipmi_kvm_ws_handshake_ok||0;'
        . 'var failed=window.__ipmi_kvm_ws_handshake_failed||0;'
        . 'var frames=window.__ipmi_kvm_ws_frame_count||0;'
        . 'var last=window.__ipmi_kvm_ws_last_msg_ts||0;'
        . 'var now=kvmNow();'
        . 'if(kvmIloSustainedTransportEvidence())return "transport_healthy";'
        . 'if(frames>0&&(now-last)>9000)return "transport_unstable";'
        . 'if(ok>0&&failed>=2&&(failed>=ok))return "transport_unstable";'
        . 'if(frames>0&&frames<10)return "provisional_frames";'
        . 'if(ok>0&&failed>ok)return "transport_unstable";'
        . 'if(ok>0&&failed===0&&frames===0)return "handshake_ok_awaiting_frames";'
        . 'if(ok>0&&failed>0)return "mixed_handshake";'
        . 'if(attempted>0&&ok===0&&failed>0)return "all_handshakes_failed";'
        . 'if(attempted>0&&ok===0&&failed===0)return "connecting";'
        . 'if(attempted===0)return "not_attempted";'
        . 'return "unknown";}catch(e){return "unknown";}}'
        . 'function kvmIloTransportHealthSummary(){try{return{'
        . 'attempted:window.__ipmi_kvm_ws_connect_attempted||0,'
        . 'handshake_ok:window.__ipmi_kvm_ws_handshake_ok||0,'
        . 'handshake_failed:window.__ipmi_kvm_ws_handshake_failed||0,'
        . 'frame_count:window.__ipmi_kvm_ws_frame_count||0,'
        . 'close_code:window.__ipmi_kvm_ws_close_code||0,'
        . 'close_clean:window.__ipmi_kvm_ws_close_clean||false,'
        . 'verdict:kvmIloTransportHealthVerdict(),'
        . 'sustained:kvmIloSustainedTransportEvidence()?1:0,'
        . 'last_error:String(window.__ipmi_kvm_ws_last_error||"").substring(0,120)'
        . '};}catch(e){return{verdict:"error"};}}'
        ...
        . 'var tv=st?st.transportVerdict||"not_attempted":"not_attempted";'
        . 'var tHealthy=st&&st.transportHealthy;'
        . 'var tFailed=st&&st.transportFailed;'
        . 'var tFrames=st&&st.transportFrameFlow;'
        . 'var att=0,cfail=0;try{att=window.__ipmi_kvm_ws_connect_attempted||0;}catch(_a){att=0;}try{cfail=window.__ipmi_kvm_ws_construct_failed||0;}catch(_cfx){cfail=0;}'
        . 'if(load)return {ok:false,why:"loading_only_state",loading_only:1};'
        . 'if(appPl&&!live&&tk>20&&ipmiProxyIloLooksLikeWhiteScreenStall(tk,st)){try{_kvmDbg("ilo_white_screen_failure_finalized",{tick:tk,path:"application"});}catch(_wsf){}return {ok:false,why:"launch_reached_renderer_only",white_screen:1,app_path:1};}'
        . 'if(mgmt&&!live){var why="shell_only_management_ui",app=ipmiProxyIloApplicationPathLoaded();if(app&&ov)why="native_console_route_reached_but_shell_only";else if(app)why="application_loaded_console_not_reached";else if(ov)why="shell_only_management_ui";return {ok:false,why:why,shell_only:1,management_shell_still_visible:1,app_path:app?1:0,overview_shell:ov?1:0};}'
        . 'var sessR=false;try{sessR=kvmIloSessionReadyEvidence(ctx);}catch(_sr){}'
        . 'var txHard=(tv==="all_handshakes_failed"||tv==="transport_unstable");'
        . 'var txBlockLive=(txHard||tv==="provisional_frames"||tv==="handshake_ok_awaiting_frames"||tv==="mixed_handshake");'
        . 'if((tFailed||txBlockLive)&&live&&sessR){try{_kvmDbg("ilo_strong_confirmation_rejected_transport_unhealthy",{tv:tv,live:1,sessR:1});try{kvmBugSend("BROWSER","strong_confirmation_rejected_transport_unhealthy",{transport_verdict:String(tv||""),why:"transport_unhealthy_console_not_confirmed",live:1,sessR:1});}catch(_kbr){}}catch(_rtu){}return {ok:false,why:"transport_unhealthy_console_not_confirmed",transport_verdict:tv};}'
        . 'if((tFailed||txHard)&&!live){return {ok:false,why:"relay_browser_handshake_failed",transport_verdict:tv,construct_fail:cfail};}'
        . 'var sustNow=false;try{sustNow=kvmIloSustainedTransportEvidence();}catch(_sn){sustNow=false;}'
        . 'if(tHealthy&&sustNow&&live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=4){try{_kvmDbg("ilo_strong_confirmation_transport_healthy",{tv:tv,frames:tFrames?1:0,sustained:1});}catch(_th){}return {ok:true,why:"native_console_strongly_confirmed"};}return {ok:false,why:"stabilizing_transport_healthy"};}'
        . 'if(live&&sessR&&!mgmt&&!ov&&!tHealthy){if(tv==="not_attempted"&&att===0)return {ok:false,why:"relay_ws_not_attempted",transport_verdict:tv};if(att>0){return {ok:false,why:"awaiting_relay_frame_flow",transport_verdict:tv};}return {ok:false,why:"live_display_provisional_only",transport_verdict:tv};}'
        . 'if(!ws&&!live)return {ok:false,why:"no_relay_transport"};'
        . 'if(ws&&!live)return {ok:false,why:"transport_without_live_display"};'
        . 'if(live&&!ws&&!sessR)return {ok:false,why:"live_display_no_session"};'
        . 'return {ok:false,why:"not_ready"};}'
        . 'function ipmiKvmWsTop(){try{if(window.top)return window.top;}catch(e){}return window;}'
        . 'function ipmiKvmWsBump(field,delta){try{delta=delta||1;var T=ipmiKvmWsTop();T[field]=(T[field]||0)+delta;try{window[field]=T[field];}catch(e2){}}catch(e3){}}'
        . 'function ipmiKvmTransportAttemptIdCreate(){try{var T=ipmiKvmWsTop();T.__ipmi_kvm_ws_attempt_seq=(T.__ipmi_kvm_ws_attempt_seq||0)+1;return"a"+String(T.__ipmi_kvm_ws_attempt_seq);}catch(e){return"a"+String(Date.now()%1e6);}}'
        . 'function ipmiKvmBrowserSocketNormalizeUrl(u){var o={relay_url_norm:"",target_host_masked:"",target_path_masked:"",token_suffix:"",has_token:0};try{var s=String(u||"");var i=s.indexOf("token=");if(i>=0){var j=s.indexOf("&",i+6);var tok=j>=0?s.substring(i+6,j):s.substring(i+6);tok=String(tok).replace(/[^a-f0-9]/gi,"");if(tok.length>=16){o.token_suffix=tok.slice(-8).toLowerCase();o.has_token=1;}o.relay_url_norm=s.substring(0,i+6)+"****"+o.token_suffix+(j>=0?s.substring(j):"");}else{o.relay_url_norm=s.length>200?s.substring(0,200):s;}var tq=s.indexOf("target=");if(tq>=0){var enc=s.substring(tq+7);var amp=enc.indexOf("&");if(amp>=0)enc=enc.substring(0,amp);try{enc=decodeURIComponent(enc);}catch(e4){}var el=enc.toLowerCase();var sp=el.indexOf("wss://")>=0?el.indexOf("wss://"):el.indexOf("ws://");if(sp>=0){var rawU=enc.substring(sp);var skip=rawU.toLowerCase().indexOf("wss://")===0?6:5;var rest=rawU.substring(skip);var slash=rest.indexOf("/");var host=slash>=0?rest.substring(0,slash):rest;var path=slash>=0?rest.substring(slash):"/";var hp=host.split("@");var h0=hp.length>1?hp[1]:hp[0];var hc=String(h0).split(":")[0];o.target_host_masked=hc.length>2?"****"+hc.slice(-6):hc;o.target_path_masked=path.substring(0,56);}}}catch(e5){}return o;}'
        . 'function ipmiKvmBrowserRelayCorrelationAugmentUrl(u,aid){try{var s=String(u||"");if(s.toLowerCase().indexOf("browser_attempt=")>=0)return s;var id=encodeURIComponent(String(aid||""));return s.indexOf("?")>=0?s+"&browser_attempt="+id:s+"?browser_attempt="+id;}catch(e){return u;}}'
        . 'function ipmiKvmRelayWsUrlMaybeUpgrade(u,wctx){try{var s=String(u||"");if(s.indexOf("ws://")!==0)return u;var sec=false;try{sec=!!(window.top&&window.top.location&&window.top.location.protocol==="https:");}catch(e1){}try{if(!sec&&wctx&&wctx.location&&wctx.location.protocol==="https:")sec=true;}catch(e2){}try{if(!sec&&typeof location!=="undefined"&&location.protocol==="https:")sec=true;}catch(e3){}return sec?("wss://"+s.substring(6)):u;}catch(e){return u;}}'
        . 'function _kvmConEmit(lv,ev,d){try{if(window.__ipmi_kvm_console_emit)window.__ipmi_kvm_console_emit(String(lv||"log"),"ipmi_ws_relay_socket_hook",String(ev||"line"),d!=null&&typeof d==="object"?d:{msg:String(d!=null?d:"").substring(0,400)});}catch(e){}}'
        ...
        . 'kvmHookWsRelayProbe();'
        . 'function kvmHookWsRelayAllContexts(){try{var cc=collectContexts();for(var ci=0;ci<cc.length;ci++){try{kvmHookWsRelayOnWindow(cc[ci]);}catch(_hw){}}}catch(_ha){}}'
...
function ipmiProxyBuildIloKvmScript(): string
{
    return 'if(FAMILY==="ilo"){'
        ...
        . 'if(rcPage||btnFound){iloSt.anyLaunchAction=true;if(!iloSt.launchSurfaceFound){iloSt.launchSurfaceFound=true;try{_kvmDbg("ilo_launch_surface_found",{rc_page:rcPage?1:0,html5_button:btnFound?1:0});try{kvmBugSend("BROWSER","launch_surface_observed",{rc_page:rcPage?1:0,html5_button:btnFound?1:0});}catch(_lso){}}catch(_lsf){}}}'
        ...
        . 'var plx2=pathLower();var onShellPath=(plx2.indexOf("/index.html")>=0||plx2.indexOf("/restgui/")>=0||plx2==="/"||plx2==="");var shellPoll=iloSt.specShell&&!iloSt.shellPathAbandoned&&onShellPath;'
        . 'if(shellPoll&&(n===10||n===11)&&!iloSt.menuExpanded){iloSt.menuExpanded=true;for(var mx=0;mx<ctx.length;mx++){try{if(ctx[mx].document&&tryExpandIloHiddenMenus(ctx[mx].document,"shell")){_kvmDbg("ilo_launch_menu_expanded",{n:n});break;}}catch(_mex){}}}'
        . 'if(shellPoll&&n===5&&!iloSt.earlyDiscDone){iloSt.earlyDiscDone=true;var edr=tryClickIloDiscoveryLaunch(ctx,"early",iloSt);if(edr){iloSt.discTriggered=true;iloSt.anyLaunchAction=true;iloSt.clicks++;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_triggered",{how:edr,clicks:iloSt.clicks,tag:"early"});ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(_ed0){}}}'
        . 'if(shellPoll&&iloSt.menuExpanded&&n===12&&!iloSt.delayedRescanDone){iloSt.delayedRescanDone=true;var ddr=tryClickIloDiscoveryLaunch(ctx,"delayed_rescan",iloSt);if(ddr){iloSt.discTriggered=true;iloSt.anyLaunchAction=true;iloSt.clicks++;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_triggered",{how:ddr,clicks:iloSt.clicks,tag:"delayed_rescan"});ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(_dd0){}}}'
        . 'if(shellPoll&&(n%5===2)&&n>3){var dlr=tryClickIloDiscoveryLaunch(ctx,"scan",iloSt);if(dlr){iloSt.discTriggered=true;iloSt.anyLaunchAction=true;iloSt.clicks++;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_triggered",{how:dlr,clicks:iloSt.clicks});ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(_dlt){}}}'
        . 'if((n%4)===0){try{kvmHookWsRelayAllContexts();}catch(_hwc){}}'
        ...
        . 'try{'
        . 'if(kvmIloRendererOrContainerPresent(ctx[i])&&!iloSt.reported.rContainer){iloSt.reported.rContainer=true;kvmTouchProgress(iloSt);_kvmDbg("ilo_renderer_container_detected",1);try{kvmBugSend("BROWSER","renderer_container_visible",{tick:n,ctx_index:i});}catch(_rcv){}}'
        ...
        . 'if((n%2)===0){try{var _tv=kvmIloTransportHealthVerdict();iloSt.transportVerdict=_tv;if((n%8)===0){var _tvS=String(_tv||"");if(_tvS!==iloSt.bugTvTick){iloSt.bugTvTick=_tvS;try{kvmBugSend("BROWSER","browser_transport_verdict_tick",{verdict:_tvS,tick:n});}catch(_btt){}}}if(_tv==="transport_healthy"&&!iloSt.transportHealthy){iloSt.transportHealthy=true;iloSt.transportFailed=false;iloSt.transportFrameFlow=true;try{_kvmDbg("ilo_transport_health_confirmed",kvmIloTransportHealthSummary());kvmBugSend("TRANSPORT","transport_healthy",kvmIloTransportHealthSummary());}catch(_thc){}}if((_tv==="all_handshakes_failed"||_tv==="transport_unstable")&&!iloSt.transportFailed){iloSt.transportFailed=true;try{_kvmDbg("ilo_transport_health_failed",kvmIloTransportHealthSummary());kvmBugSend("TRANSPORT","transport_failed",{verdict:_tv});}catch(_thf){}}if(kvmIloWsHandshakeOk()&&!iloSt.transportHandshakeOk){iloSt.transportHandshakeOk=true;try{_kvmDbg("ilo_transport_handshake_ok",kvmIloTransportHealthSummary());}catch(_tho){}}if(kvmIloWsHandshakeFailed()&&!iloSt.transportDbgFail){iloSt.transportDbgFail=true;try{_kvmDbg("ilo_transport_handshake_failed",kvmIloTransportHealthSummary());}catch(_thdf){}}if(kvmIloWsFrameFlowActive()&&!iloSt.transportDbgFirst){iloSt.transportDbgFirst=true;try{_kvmDbg("ilo_transport_frame_flow_started",kvmIloTransportHealthSummary());}catch(_tff){}}if(ipmiProxyIloHasLiveDisplayEvidence(window)&&iloSt.reported.sessR&&!iloSt.transportHealthy&&(n%16)===0){try{_kvmDbg("ilo_transport_health_provisional",{transport_verdict:_tv,live_display:1,session_ready:1,relay_only_metrics:1});kvmBugSend("BROWSER","strong_confirmation_rejected_transport_unhealthy",{transport_verdict:_tv});}catch(_tp){}}}catch(_tve){}}'
        . 'var escSrv=(!!iloSt.serverForcedShellAbandon&&shellPoll&&!rcPage&&n>=1);var escEarly=(iloSt.shellLaunchProvenIneffective&&shellPoll&&!rcPage&&n>=3);var escLate=(iloSt.specShell&&n>=10&&shellPoll&&!rcPage);if(iloSt.specShell&&!iloSt.shellEscalationConsumed&&!wsNow&&(escSrv||escEarly||escLate)){var _ek=tryShellDiscoveryEscalationOnce(ctx,iloSt);if(_ek==="href"){ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}if(_ek==="spa"){return;}}'
        ...
        . 'if(kvmIloSessionReadyEvidence(window)&&!iloSt.reported.sessR){iloSt.reported.sessR=true;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_session_ready_evidence_detected",1);_kvmDbg("ilo_console_session_ready",1);kvmBugSend("BROWSER","session_ready_heuristic",{tick:n});}catch(es0){}}'
        ...
        . 'if(appPathNow&&!iloSt.reported.appPath){iloSt.reported.appPath=true;try{_kvmDbg("ilo_application_path_loaded",{path:pathLower()});kvmBugSend("BROWSER","application_path_loaded",{path:String(pathLower()).substring(0,160)});var _b0=ipmiProxyIloShellVsConsoleStateLoad()||{};_b0.application_path_loaded=1;_b0.app_tick=n;ipmiProxyIloShellVsConsoleStateStore(_b0);}catch(_ap0){}}'
        ...
        . 'if((n%4)===0){try{var _mxTk=iloSt.__lastTick||0;var _mxWs=!!kvmIloWsTransportEvidence();var _mxLive=!!ipmiProxyIloHasLiveDisplayEvidence(window);var _mxApp=ipmiProxyIloApplicationPathLoaded();var _ths=null;try{_ths=kvmIloTransportHealthSummary();}catch(_thse){_ths={verdict:"error"};}
_kvmDbg("ilo_kvm_debug_matrix",{launch_function_found:iloSt.launchFunctionFound?1:0,launch_function_effective:iloSt.launchFunctionEffective?1:0,shell_path_abandoned:iloSt.shellPathAbandoned?1:0,application_navigation_committed:iloSt.appNavCommitted?1:0,application_navigation_failed:iloSt.appNavFailed?1:0,white_screen_stall:iloSt.whiteStallReported?1:0,application_path_loaded:_mxApp?1:0,live_display_visible:_mxLive?1:0,transport_started:_mxWs?1:0,transport_verdict:String(iloSt.transportVerdict||"not_attempted"),transport_healthy:iloSt.transportHealthy?1:0,transport_failed:iloSt.transportFailed?1:0,transport_handshake_ok:iloSt.transportHandshakeOk?1:0,transport_frame_flow:iloSt.transportFrameFlow?1:0,sustained_transport:kvmIloSustainedTransportEvidence()?1:0,session_ready_evidence:iloSt.reported.sessR?1:0,ws_connect_attempted:_ths?_ths.attempted||0:0,ws_handshake_ok_count:_ths?_ths.handshake_ok||0:0,ws_handshake_fail_count:_ths?_ths.handshake_failed||0:0,ws_frame_count:_ths?_ths.frame_count||0:0,relay_upstream_ws_diag:"server_error_log_only",strong_confirmation_ready:rf.ok?1:0,strong_confirmation_why:String(rf.why||""),privilege_verdict:String(iloSt.privilegeVerdict||"unknown"),privilege_block:iloSt.privilegeBlockDetected?1:0,privilege_block_finalized:iloSt.privilegeBlockFinalized?1:0,visible_user:String(iloSt.privilegeUserState||"unknown").substring(0,40),html5_button_found:iloSt.html5LaunchControlFound?1:0,html5_launch_attempted:iloSt.html5LaunchAttempted?1:0,html5_launch_effective:iloSt.html5LaunchEffective?1:0,shell_launch_probe_exhausted:iloSt.shellLaunchProbeExhausted?1:0,launch_attempted:iloSt.anyLaunchAction?1:0});}catch(_mx){}}'
        . 'if(rf.ok&&!iloSt.reported.fin){iloSt.reported.fin=true;iloSt.lastVerdict="console_interactive_confirmed";try{var _fths=null;try{_fths=kvmIloTransportHealthSummary();}catch(_fe){_fths={verdict:"unknown"};}ipmiProxyIloFinalizeStrongConfirmationFromVisibleUi(window,iloSt,rf);try{var _sustFin=false;try{_sustFin=kvmIloSustainedTransportEvidence();}catch(_sf){_sustFin=false;}kvmBugSend("FINAL","kvm_final_summary",{verdict:"native_console_strongly_confirmed",strong_confirmation:1,application_path_loaded:appPathNow?1:0,shell_path_abandoned:iloSt.shellPathAbandoned?1:0,launch_attempted:iloSt.anyLaunchAction?1:0,browser_ws_handshake_ok:iloSt.transportHandshakeOk?1:0,frame_pump_started:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,first_frame_observed:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,sustained_frame_flow_observed:_sustFin?1:0,sustained_transport_ok:_sustFin?1:0,frame_pump_active:((_fths&&(_fths.frame_count|0)>0)||((window.__ipmi_kvm_ws_frame_count|0)>0))?1:0,transport_healthy:iloSt.transportHealthy?1:0,session_ready:iloSt.reported.sessR?1:0,live_display:liveNow?1:0,transport_why:String(rf.why||""),transport_verdict:String(iloSt.transportVerdict||""),ws_connect_attempted:(window.__ipmi_kvm_ws_connect_attempted||0),ws_frame_count:_fths?_fths.frame_count||0:0});try{setTimeout(function(){try{kvmBugSyncTokenGlobally();kvmBugFinalizeRun({reason:"strong_confirm_delay"});}catch(_z){}},2800);}catch(_z2){}}catch(_kbf){}_kvmDbg("ilo_strong_confirmation_achieved",{why:rf.why||"",transport_verdict:iloSt.transportVerdict||"",transport_healthy:iloSt.transportHealthy?1:0});_kvmDbg("ilo_native_console_strongly_confirmed",{why:rf.why||"",frame_count:_fths?_fths.frame_count||0:0,handshake_ok:_fths?_fths.handshake_ok||0:0});_kvmDbg("ilo_user_visible_console_success",{why:rf.why||"",strong_confirmation:1,live_display:liveNow?1:0,transport:wsNow?1:0,transport_verdict:iloSt.transportVerdict||"",transport_healthy:iloSt.transportHealthy?1:0,ws_frame_count:_fths?_fths.frame_count||0:0});_kvmDbg("ilo_confirmation_strong",{tier:"native_console_strongly_confirmed",source:"finalize",transport_why:rf.why||""});_kvmDbg("ilo_console_interactive_confirmed",rf);_kvmDbg("ilo_console_readiness_verdict",{verdict:"native_console_strongly_confirmed",why:rf.why||"",native_console_confirmation:"native_console_strongly_confirmed",live_display:liveNow?1:0,transport_healthy:iloSt.transportHealthy?1:0,transport_verdict:iloSt.transportVerdict||"",management_shell_visible:mgmtNow?1:0,overview_shell:overviewNow?1:0});markDone();return;}catch(emf){}}'
        ...
        . 'else if(rAny&&!liveNow&&!wsNow&&n>10)vNext="launch_reached_renderer_only";'
        ...
        . 'else if(iloSt.reported.sessR&&!iloSt.reported.fin)vNext="console_session_ready_provisional";'
        ...
        . 'if(iloSt.lastVerdict!==iloSt.prevVerdict){iloSt.prevVerdict=iloSt.lastVerdict;try{var _ltf=(iloSt.anyLaunchAction||rcPage||btnFound||iloSt.launchSurfaceFound)?1:0;var _lat=(iloSt.discTriggered||iloSt.discNavTriggered)?1:0;var _lae=iloSt.launchNoEffectReported?0:1;var _tHint=(iloSt.transportHealthy&&liveNow&&!ldNow&&!mgmtNow&&!overviewNow)?"native_console_strongly_confirmed":(routeNow&&!liveNow?"application_loaded_console_not_reached":"native_console_not_confirmed");if(appPathNow&&!liveNow&&overviewNow)_tHint="native_console_route_reached_but_shell_only";else if(appPathNow&&!liveNow&&!overviewNow)_tHint="application_loaded_console_not_reached";if(overviewNow&&!liveNow&&!appPathNow)_tHint="shell_only_management_ui";if(routeNow&&!liveNow&&!mgmtNow)_tHint="native_console_route_reached_not_ready";_kvmDbg("ilo_console_readiness_verdict",{verdict:iloSt.lastVerdict,ws:wsNow?1:0,transport_evidence:wsNow?1:0,transport_started:wsNow?1:0,session_ready:iloSt.reported.sessR?1:0,load:ldNow?1:0,loading_persisted:(ldNow&&ldMs>12000)?1:0,loading_ms:ldNow?ldMs:0,rContainer:rAny?1:0,renderer_container:rAny?1:0,session_ready_evidence:iloSt.reported.sessR?1:0,sess:iloSt.reported.sessR?1:0,speculative_shell:iloSt.specShell?1:0,shell_autolaunch_initial_phase:(iloSt.lastVerdict==="shell_autolaunch_allowed")?1:0,launch_discovery_started:iloSt.discoveryStarted?1:0,launch_surface_found:iloSt.launchSurfaceFound?1:0,launch_target_found:_ltf,launch_action_triggered:_lat,launch_action_effective:_lae,launch_outcome_probe_pending:(iloSt.pendingOutcomeCheck>0)?1:0,launch_discovery:iloSt.discoveryStarted?1:0,any_launch_action:iloSt.anyLaunchAction?1:0,clicks:iloSt.clicks,disc_nav:iloSt.discNavTriggered?1:0,shell_escalation_consumed:iloSt.shellEscalationConsumed?1:0,white_screen_stall:iloSt.whiteStallReported?1:0,final_discovery_verdict:iloSt.lastVerdict,shell_only_ui:shellOnlyNow?1:0,management_shell_visible:mgmtNow?1:0,overview_shell:overviewNow?1:0,application_path_loaded:appPathNow?1:0,application_loaded_shell_only:ipmiProxyIloApplicationLoadedShellOnly(window)?1:0,helper_activity:helperNow?1:0,helper_aided_discovery:iloSt.helperAidedDiscovery?1:0,helper_correlated_with_visible_console:ipmiProxyIloHelperActivityCorrelatesWithConsoleReach(window)?1:0,live_display_evidence:liveNow?1:0,native_route_reached:routeNow?1:0,native_console_tier_hint:_tHint});}catch(ev){}}'
        . 'if(iloSt.stall){return;}'
        . 'if(hitMax){try{_kvmDbg("ilo_console_stalled",{ticks:n,reason:"max_ticks",launch_discovery_failed:iloSt.launchDiscoveryFailed?1:0,no_transport:!iloSt.reported.trans?1:0});var _hitV="transport_unhealthy_console_not_confirmed";var _sustHit=false;try{_sustHit=kvmIloSustainedTransportEvidence();}catch(_ehs){_sustHit=false;}if(iloSt.transportHealthy&&_sustHit)_hitV="console_transport_healthy";else if(iloSt.transportHealthy&&!_sustHit)_hitV="transport_unstable_console_not_confirmed";else if(iloSt.launchDiscoveryFailed)_hitV="launch_discovery_failed";else if(iloSt.launchNoEffectReported)_hitV="launch_action_no_effect";try{kvmBugSend("FINAL","kvm_final_summary",{verdict:_hitV,strong_confirmation:0,application_path_loaded:appPathNow?1:0,shell_path_abandoned:iloSt.shellPathAbandoned?1:0,launch_attempted:iloSt.anyLaunchAction?1:0,browser_ws_handshake_ok:iloSt.transportHandshakeOk?1:0,frame_pump_started:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,first_frame_observed:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,sustained_frame_flow_observed:_sustHit?1:0,sustained_transport_ok:_sustHit?1:0,frame_pump_active:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,transport_healthy:((iloSt.transportHealthy&&_sustHit)?1:0),session_ready:iloSt.reported.sessR?1:0,live_display:liveNow?1:0,transport_verdict:String(iloSt.transportVerdict||""),final_failure_reason:String(iloSt.transportVerdict||""),ws_connect_attempted:(window.__ipmi_kvm_ws_connect_attempted||0),reason:"max_ticks_deferred_final",note:"[FINAL] flush via relay terminal idle close or pagehide; avoids stale block before late relay lines"});}catch(_kbm){}try{setTimeout(function(){try{kvmBugFinalizeRun({reason:"max_ticks",force_finalize:1});}catch(_fz){}},14000);}catch(_z){}if(iloSt.launchDiscoveryFailed){_kvmDbg("ilo_console_readiness_verdict",{verdict:"launch_discovery_failed",reason:"max_ticks",transport_started:wsNow?1:0,launch_target_found:(iloSt.anyLaunchAction||rcPage||btnFound)?1:0,launch_action_triggered:(iloSt.discTriggered||iloSt.discNavTriggered)?1:0});}}catch(em){}return;}'
        . 'setTimeout(tick,250);'
        . '})();'
        . 'return;'
        . '}';
 ...
 */
function ipmiProxyBuildIloHelperRuntimeJs(string $pxJs, string $dbgLit, ?array $kvmBuglog = null): string
{
    ...
    return '(function(){'
        . 'var P=' . $pxJs . ';var DBG=' . $dbgLit . ';var KVM_BUGLOG=' . $kbJs . ';'
        ...
        . ipmiProxyKvmConsoleCaptureAndDomGuardJs()
        . 'try{if(typeof window.addEventListener==="function"){window.addEventListener("pagehide",function(){try{kvmBugSyncTokenGlobally();kvmBugFinalizeRun({reason:"helper_pagehide",force_finalize:1});}catch(_phh){}},true);}}catch(_pehh){}'
        ...
        . 'try{kvmBugSend("HELPER","helper_page_loaded",{path:String(location.pathname||"").substring(0,160)});}catch(_hb){}'
...
function ipmiProxyInjectKvmAutoLaunchPatch(string $html, string $token, $sessionOrLegacyBmcType, bool $kvmAutoFlow = false, ?array $launchPlan = null, ?mysqli $persistKvmPlanMysqli = null, ?array &$injectOut = null, string $bmcPath = ''): string
{
    ...
    $planLite['kvm_shell_abandon'] = (isset($metaLite['kvm_shell_abandon']) && is_array($metaLite['kvm_shell_abandon']))
        ...
        : null;
    $kvmBugForHelper = $planLite['kvm_buglog'];
    ...
    if (ipmiProxyShouldInjectHelperRuntime($patchMode)) {
        $helperBody = ipmiProxyBuildIloHelperRuntimeJs($pxJs, $dbgLit, is_array($kvmBugForHelper) ? $kvmBugForHelper : null);
        $injectMeta = [
            'mode'      => 'helper_minimal',
            'js_ok'     => true,
            'js_reason' => '',
...
if ($httpCode === 403) {
    $iloType403 = ipmiWebNormalizeBmcType((string) ($session['bmc_type'] ?? 'generic'));
    if (ipmiWebIsNormalizedIloType($iloType403)) {
        $path403 = strtolower(rtrim((string) parse_url($bmcPath, PHP_URL_PATH), '/'));
        if ($path403 === '/sse/ui' || str_ends_with($path403, '/sse/ui')) {
            ipmiProxyIloAbandonShellPath($mysqli, $token, 'SHELL_SSE_403', $bmcPath);
        } elseif ($path403 === '/json/session_info' || str_ends_with($path403, '/json/session_info')) {
            ipmiProxyIloAbandonShellPath($mysqli, $token, 'SHELL_SESSION_INFO_403', $bmcPath);
        }
    }
}
