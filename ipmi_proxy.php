...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'if(live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=3){try{_kvmDbg("ilo_strong_confirmation_live_display_session_ready",{ws:ws?1:0,live:1,sessR:1});}catch(_sc){}return {ok:true,why:"user_visible_console_success"};}return {ok:false,why:"stabilizing"};}'
        . 'if(!ws&&!live)return {ok:false,why:"no_relay_transport"};'
        . 'if(ws&&!live)return {ok:false,why:"transport_without_live_display"};'
        . 'if(live&&!ws){st.finStable=(st.finStable||0)+1;if(st.finStable>=5){try{_kvmDbg("ilo_strong_confirmation_live_display_without_relay",{ws:0,live:1});}catch(_sc2){}return {ok:true,why:"user_visible_console_success_without_relay"};}return {ok:false,why:"live_display_stabilizing_no_relay"};}'
        ...
        . 'w.WebSocket=function(u,p){try{if(typeof u==="string"&&(u.indexOf("ipmi_ws_relay.php")>=0||u.indexOf("wss://")>=0||u.indexOf("ws://")>=0)){try{window.__ipmi_kvm_ws_relay_ts=kvmNow();}catch(_ts0){}try{w.__ipmi_kvm_ws_relay_ts=kvmNow();}catch(_ts1){}try{_kvmDbg("ipmi_ws_relay_client_open",String(u).substring(0,120));}catch(e1){}}}catch(e2){}'
        . 'return new W0(u,p);};try{w.WebSocket.prototype=W0.prototype;}catch(_wp){}'
        . '}catch(e3){}}'
