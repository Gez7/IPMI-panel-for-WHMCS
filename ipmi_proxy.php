...
function ipmiProxyBuildKvmAutoLaunchPreambleJs(string $familyJs, string $planJs, string $pxJs, string $autoJs, string $dbgLit): string
{
    return '(function(){'
        ...
        . 'function kvmBugToken(){try{var m=String(P||"").match(/\\/([a-f0-9]{64})(?:\\/|$)/i);return m?m[1].toLowerCase():"";}catch(e){return"";}}'
        . 'function kvmBugSend(section,event,detail){try{if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugToken();if(!tok||tok.length!==64)return;var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:String(section||"BROWSER"),event:String(event||""),detail:(detail!=null&&typeof detail==="object")?detail:{note:String(detail||"")}});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bb=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bb))return;}catch(e1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(e2){}}catch(e){}}'
...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'var closeCode=window.__ipmi_kvm_ws_close_code||0;'
        . 'if(frames>0)return "transport_healthy";'
        . 'if(ok>0&&failed>ok)return "transport_unstable";'
        . 'if(ok>0&&failed===0)return "handshake_ok_awaiting_frames";'
        ...
        . 'var txBad=(tv==="all_handshakes_failed"||tv==="transport_unstable");'
        . 'if((tFailed||txBad)&&live&&sessR){try{_kvmDbg("ilo_strong_confirmation_rejected_transport_unhealthy",{tv:tv,live:1,sessR:1});}catch(_rtu){}return {ok:false,why:"transport_unhealthy_console_not_confirmed",transport_verdict:tv};}'
        . 'if((tFailed||txBad)&&!live){return {ok:false,why:"relay_browser_handshake_failed",transport_verdict:tv,construct_fail:cfail};}'
        . 'if(tHealthy&&live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=4){try{_kvmDbg("ilo_strong_confirmation_transport_healthy",{tv:tv,frames:tFrames?1:0});}catch(_th){}return {ok:true,why:"native_console_strongly_confirmed"};}return {ok:false,why:"stabilizing_transport_healthy"};}'
        ...
        . 'w.WebSocket=function(u,p){var isRelay=(typeof u==="string"&&u.indexOf("ipmi_ws_relay.php")>=0);'
        . 'var sock;try{sock=new W0(u,p);}catch(ce){if(isRelay){try{window.__ipmi_kvm_ws_construct_failed=(window.__ipmi_kvm_ws_construct_failed||0)+1;window.__ipmi_kvm_ws_handshake_failed=(window.__ipmi_kvm_ws_handshake_failed||0)+1;window.__ipmi_kvm_ws_last_error=String(ce.message||"").substring(0,200);_kvmDbg("ipmi_ws_relay_construct_error",{url:String(u).substring(0,120),err:String(ce.message||"").substring(0,120)});kvmBugSend("TRANSPORT","browser_ws_construct_failed",{err:String(ce.message||"").substring(0,120)});}catch(_ce){}}throw ce;}'
        . 'if(isRelay){try{window.__ipmi_kvm_ws_relay_ts=kvmNow();w.__ipmi_kvm_ws_relay_ts=kvmNow();window.__ipmi_kvm_ws_connect_attempted=(window.__ipmi_kvm_ws_connect_attempted||0)+1;_kvmDbg("ipmi_ws_relay_client_open",{url:String(u).substring(0,120),attempt:window.__ipmi_kvm_ws_connect_attempted||1});}catch(_ts){}'
        . 'try{sock.addEventListener("open",function(){try{window.__ipmi_kvm_ws_handshake_ok=(window.__ipmi_kvm_ws_handshake_ok||0)+1;window.__ipmi_kvm_ws_open_ts=kvmNow();_kvmDbg("ipmi_ws_relay_handshake_ok",{url:String(u).substring(0,120),ok_count:window.__ipmi_kvm_ws_handshake_ok||1});kvmBugSend("TRANSPORT","browser_ws_handshake_succeeded",{url:String(u).substring(0,96)});}catch(_o){}});}catch(_ol){}'
        . 'try{sock.addEventListener("error",function(){try{window.__ipmi_kvm_ws_handshake_failed=(window.__ipmi_kvm_ws_handshake_failed||0)+1;_kvmDbg("ipmi_ws_relay_handshake_error",{url:String(u).substring(0,120),fail_count:window.__ipmi_kvm_ws_handshake_failed||1});kvmBugSend("TRANSPORT","browser_ws_handshake_failed_event",{fail_count:window.__ipmi_kvm_ws_handshake_failed||1});}catch(_e){}});}catch(_el){}'
        . 'try{sock.addEventListener("close",function(ev){try{var code=ev?ev.code||0:0;var clean=!!(ev&&ev.wasClean);window.__ipmi_kvm_ws_close_code=code;window.__ipmi_kvm_ws_close_clean=clean;_kvmDbg("ipmi_ws_relay_closed",{code:code,clean:clean?1:0,url:String(u).substring(0,120)});kvmBugSend("TRANSPORT","browser_ws_closed",{code:code,clean:clean?1:0});}catch(_cl){}});}catch(_cll){}'
        . 'try{sock.addEventListener("message",function(){try{window.__ipmi_kvm_ws_frame_count=(window.__ipmi_kvm_ws_frame_count||0)+1;if(window.__ipmi_kvm_ws_frame_count===1){window.__ipmi_kvm_ws_first_frame_ts=kvmNow();_kvmDbg("ipmi_ws_relay_first_frame",{ts:kvmNow()});kvmBugSend("TRANSPORT","browser_ws_first_application_frame",{});}}catch(_m){}});}catch(_ml){}'
...
_kvmDbg("ilo_kvm_debug_matrix",{launch_function_found:iloSt.launchFunctionFound?1:0,launch_function_effective:iloSt.launchFunctionEffective?1:0,shell_path_abandoned:iloSt.shellPathAbandoned?1:0,application_navigation_committed:iloSt.appNavCommitted?1:0,application_navigation_failed:iloSt.appNavFailed?1:0,white_screen_stall:iloSt.whiteStallReported?1:0,live_display_visible:_mxLive?1:0,transport_started:_mxWs?1:0,transport_verdict:String(iloSt.transportVerdict||"not_attempted"),transport_healthy:iloSt.transportHealthy?1:0,transport_failed:iloSt.transportFailed?1:0,transport_handshake_ok:iloSt.transportHandshakeOk?1:0,transport_frame_flow:iloSt.transportFrameFlow?1:0,ws_connect_attempted:_ths?_ths.attempted||0:0,ws_handshake_ok_count:_ths?_ths.handshake_ok||0:0,ws_handshake_fail_count:_ths?_ths.handshake_failed||0:0,ws_frame_count:_ths?_ths.frame_count||0:0,relay_upstream_ws_diag:"server_error_log_only",strong_confirmation_ready:rf.ok?1:0,strong_confirmation_why:String(rf.why||""),privilege_verdict:String(iloSt.privilegeVerdict||"unknown"),privilege_block:iloSt.privilegeBlockDetected?1:0,privilege_block_finalized:iloSt.privilegeBlockFinalized?1:0,visible_user:String(iloSt.privilegeUserState||"unknown").substring(0,40),html5_button_found:iloSt.html5LaunchControlFound?1:0,html5_launch_attempted:iloSt.html5LaunchAttempted?1:0,html5_launch_effective:iloSt.html5LaunchEffective?1:0,shell_launch_probe_exhausted:iloSt.shellLaunchProbeExhausted?1:0});}catch(_mx){}}'
        ...
        . 'if(hitMax){try{_kvmDbg("ilo_console_stalled",{ticks:n,reason:"max_ticks",launch_discovery_failed:iloSt.launchDiscoveryFailed?1:0,no_transport:!iloSt.reported.trans?1:0});var _hitV="transport_unhealthy_console_not_confirmed";if(iloSt.transportHealthy)_hitV="console_transport_healthy";else if(iloSt.launchDiscoveryFailed)_hitV="launch_discovery_failed";else if(iloSt.launchNoEffectReported)_hitV="launch_action_no_effect";try{kvmBugSend("FINAL","kvm_final_summary",{verdict:_hitV,strong_confirmation:0,application_path_loaded:appPathNow?1:0,shell_path_abandoned:iloSt.shellPathAbandoned?1:0,launch_attempted:iloSt.anyLaunchAction?1:0,browser_ws_handshake_ok:iloSt.transportHandshakeOk?1:0,upstream_ws_ok:(iloSt.transportHealthy&&iloSt.transportFrameFlow)?1:0,frame_pump_active:((window.__ipmi_kvm_ws_frame_count|0)>0)?1:0,transport_healthy:iloSt.transportHealthy?1:0,session_ready:iloSt.reported.sessR?1:0,live_display:liveNow?1:0,transport_verdict:String(iloSt.transportVerdict||""),reason:"max_ticks"});}catch(_kbm){}if(iloSt.launchDiscoveryFailed){_kvmDbg("ilo_console_readiness_verdict",{verdict:"launch_discovery_failed",reason:"max_ticks",transport_started:wsNow?1:0,launch_target_found:(iloSt.anyLaunchAction||rcPage||btnFound)?1:0,launch_action_triggered:(iloSt.discTriggered||iloSt.discNavTriggered)?1:0});}}catch(em){}return;}'
        . 'setTimeout(tick,250);'
        . '})();'
        . 'return;'
        . '}';
