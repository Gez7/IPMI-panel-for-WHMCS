...
function ipmiProxyBuildKvmAutoLaunchIloDomHelpersJs(): string
{
    return ''
        ...
        . 'function tryShellDiscoveryEscalationOnce(ctxArr,st){try{if(!st||st.shellEscalationConsumed)return"";if(kvmIloWsTransportEvidence())return"";st.shellEscalationConsumed=true;try{_kvmDbg("ilo_launch_discovery_escalation_allowed",{reason:"shell_index_no_transport"});}catch(_ea){}if(tryFollowBoundedConsoleHref(ctxArr,st)){st.discNavTriggered=true;st.anyLaunchAction=true;st.discoveryEscalationKind="console_href";try{_kvmDbg("ilo_launch_discovery_escalation_attempted",{kind:"console_href_ok"});}catch(_ok){}return"href";}try{_kvmDbg("ilo_launch_discovery_escalation_attempted",{kind:"spa_bootstrap_fallback",target:"html/application.html"});go("/html/application.html?ipmi_kvm_auto=1");st.discNavTriggered=true;st.anyLaunchAction=true;st.discoveryEscalationKind="spa_bootstrap";try{_kvmDbg("ilo_launch_navigation_triggered",{target:"html/application.html"});}catch(_nt){}markAppRedirected();return"spa";}catch(_ge){try{_kvmDbg("ilo_launch_discovery_escalation_failed",{err:String(_ge)});}catch(_ef){}}return"";}catch(_se){return"";}}'
...
function ipmiProxyBuildIloKvmScript(): string
{
    return 'if(FAMILY==="ilo"){'
        ...
        . 'if(!started&&iloSt.phase<=1){for(var b=0;b<ctx.length;b++){if(callStart(ctx[b])){started=true;iloSt.anyLaunchAction=true;iloSt.discTriggered=true;iloSt.clicks++;iloSt.phase=2;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_function_invoked","renderer_ctx");_kvmDbg("ilo_launch_triggered",{how:"callStart_ctx",clicks:iloSt.clicks});if(iloSt.specShell)ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(e1){}break;}}}'
        . 'if(!started&&iloSt.phase<=2){for(var h=0;h<ctx.length;h++){if(callIloStart(ctx[h])){started=true;iloSt.anyLaunchAction=true;iloSt.discTriggered=true;iloSt.clicks++;iloSt.phase=3;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_function_invoked","ilo_start");_kvmDbg("ilo_launch_triggered",{how:"callIloStart",clicks:iloSt.clicks});if(iloSt.specShell)ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(e2){}break;}}}'
        ...
        . 'if(!rcReady&&iloSt.phase===0&&n<=16){var dtEarly=getIloDirectTopRenderer(window);if(dtEarly){_kvmDbg("ilo_direct_renderer_attempt",1);if(callStart(dtEarly)){iloSt.phase=1;iloSt.anyLaunchAction=true;iloSt.discTriggered=true;iloSt.clicks++;kvmTouchProgress(iloSt);try{_kvmDbg("ilo_launch_function_invoked","direct_early");_kvmDbg("ilo_launch_triggered",{how:"callStart_early",clicks:iloSt.clicks});if(iloSt.specShell)ipmiProxyIloArmLaunchOutcomeProbe(iloSt,n,sigEarly,wsEarly,liveEarly);}catch(e5){}}}}'
        . 'var hitMax=(n>=max);'
