...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'var att=0;try{att=window.__ipmi_kvm_ws_connect_attempted||0;}catch(_a){att=0;}'
        . 'if(load)return {ok:false,why:"loading_only_state",loading_only:1};'
        . 'if(appPl&&!live&&tk>20&&ipmiProxyIloLooksLikeWhiteScreenStall(tk,st)){try{_kvmDbg("ilo_white_screen_failure_finalized",{tick:tk,path:"application"});}catch(_wsf){}return {ok:false,why:"application_loaded_white_screen",white_screen:1,app_path:1};}'
        ...
        . 'if((tFailed||txBad)&&!live)return {ok:false,why:"relay_handshake_failed",transport_verdict:tv};'
        . 'if(tHealthy&&live&&sessR&&!mgmt&&!ov){st.finStable=(st.finStable||0)+1;if(st.finStable>=4){try{_kvmDbg("ilo_strong_confirmation_transport_healthy",{tv:tv,frames:tFrames?1:0});}catch(_th){}return {ok:true,why:"console_transport_healthy"};}return {ok:false,why:"stabilizing_transport_healthy"};}'
        ...
        . 'var sock;try{sock=new W0(u,p);}catch(ce){if(isRelay){try{window.__ipmi_kvm_ws_handshake_failed=(window.__ipmi_kvm_ws_handshake_failed||0)+1;window.__ipmi_kvm_ws_last_error=String(ce.message||"").substring(0,200);_kvmDbg("ipmi_ws_relay_construct_error",{url:String(u).substring(0,120),err:String(ce.message||"").substring(0,120)});}catch(_ce){}}throw ce;}'
        ...
        . 'try{sock.addEventListener("open",function(){try{window.__ipmi_kvm_ws_handshake_ok=(window.__ipmi_kvm_ws_handshake_ok||0)+1;window.__ipmi_kvm_ws_open_ts=kvmNow();_kvmDbg("ipmi_ws_relay_handshake_ok",{url:String(u).substring(0,120),ok_count:window.__ipmi_kvm_ws_handshake_ok||1});}catch(_o){}});}catch(_ol){}'
        . 'try{sock.addEventListener("error",function(){try{window.__ipmi_kvm_ws_handshake_failed=(window.__ipmi_kvm_ws_handshake_failed||0)+1;_kvmDbg("ipmi_ws_relay_handshake_error",{url:String(u).substring(0,120),fail_count:window.__ipmi_kvm_ws_handshake_failed||1});}catch(_e){}});}catch(_el){}'
        . 'try{sock.addEventListener("close",function(ev){try{var code=ev?ev.code||0:0;var clean=!!(ev&&ev.wasClean);window.__ipmi_kvm_ws_close_code=code;window.__ipmi_kvm_ws_close_clean=clean;_kvmDbg("ipmi_ws_relay_closed",{code:code,clean:clean?1:0,url:String(u).substring(0,120)});}catch(_cl){}});}catch(_cll){}'
        . 'try{sock.addEventListener("message",function(){try{window.__ipmi_kvm_ws_frame_count=(window.__ipmi_kvm_ws_frame_count||0)+1;if(window.__ipmi_kvm_ws_frame_count===1){window.__ipmi_kvm_ws_first_frame_ts=kvmNow();_kvmDbg("ipmi_ws_relay_first_frame",{ts:kvmNow()});}}catch(_m){}});}catch(_ml){}'
