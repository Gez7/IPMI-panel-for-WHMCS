...
/**
 * Optional KVM capability map generated by fleet probe.
 * CSV: /var/www/html/data/kvm_availability.csv (preferred),
 *      /tmp/kvm_availability.csv (fallback)
 */
$kvmAvailabilityByServer = [];
$kvmAvailabilityLoaded = false;
$kvmAvailabilityCsvCandidates = [
    __DIR__ . '/data/kvm_availability.csv',
    '/tmp/kvm_availability.csv',
];
foreach ($kvmAvailabilityCsvCandidates as $kvmAvailabilityCsv) {
    if (!is_readable($kvmAvailabilityCsv)) {
        continue;
    }
    $fp = @fopen($kvmAvailabilityCsv, 'r');
    if ($fp === false) {
        continue;
    }
    // Skip header
    ...
    while (($cols = fgetcsv($fp)) !== false) {
        ...
        $sid = (int) ($cols[0] ?? 0);
        $result = strtoupper(trim((string) ($cols[3] ?? '')));
        if ($sid > 0 && $result !== '') {
            $kvmAvailabilityByServer[$sid] = $result;
        }
    }
    fclose($fp);
    if ($kvmAvailabilityByServer !== []) {
        $kvmAvailabilityLoaded = true;
        break;
    }
}
    ...
    <table>
      ...
      <?php while ($s = $result->fetch_assoc()): ?>
        <?php
          ...
          $kvmDebugHref = $kvmHref . '&debug=1';
          $kvmAvailability = (string) ($kvmAvailabilityByServer[$id] ?? '');
          $kvmIsAvailable = ($kvmAvailability === 'HTML5_READY');
          $kvmKnown = ($kvmAvailability !== '');
        ?>
        <tr>
          ...
          <td>
            <?php if ($isSuspended && !$isAdmin): ?>
              <span class="btn-kvm btn-kvm-unavailable" style="opacity:.55;cursor:not-allowed;">
                <?php if ($kvmKnown): ?>
                  <?= $kvmIsAvailable ? 'KVM Available (Blocked)' : 'KVM Not Available (Blocked)' ?>
                <?php else: ?>
                  KVM Status Unknown (Blocked)
                <?php endif; ?>
              </span>
            <?php else: ?>
              <?php if ($kvmKnown && $kvmIsAvailable): ?>
                <a href="<?= htmlspecialchars($kvmHref, ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="btn-kvm btn-kvm-available">KVM Available</a>
              <?php elseif ($kvmKnown): ?>
                <span class="btn-kvm btn-kvm-unavailable" title="Browser-native KVM is not currently available for this server.">KVM Not Available</span>
              <?php else: ?>
                <a href="<?= htmlspecialchars($kvmHref, ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="btn-kvm btn-kvm-unknown" title="KVM capability map not loaded; opening KVM directly.">KVM Status Unknown</a>
              <?php endif; ?>
              <a href="<?= htmlspecialchars($kvmDebugHref, ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="btn-kvm" style="margin-left:6px;">Debug</a>
            <?php endif; ?>
          </td>
          <?php if ($isAdmin): ?>
